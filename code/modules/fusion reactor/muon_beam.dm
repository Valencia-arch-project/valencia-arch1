/obj/machinery/power/muon_beam
	name = "Muon beam emitter"
	desc = "It is a heavy duty muon beam emitter"
	icon = 'icons/obj/fusion_reactor.dmi'
	//icon_state = "muon"
	anchored = 1
	density = 1
...
/obj/machinery/power/muon_beam/initialize()
	..()
	if(state == 2 && anchored)
		connect_to_network()

/obj/machinery/power/muon_beam/update_icon()
	if (active && powernet && avail(active_power_usage))
		//icon_state = "muon_+a"
	else
		//icon_state = "muon"

/obj/machinery/power/muon_beam/attack_hand(mob/user as mob)
	src.add_fingerprint(user)
	activate(user)

/obj/machinery/power/muon_beam/proc/activate(mob/user as mob)
	if(state == 2)
		if(!powernet)
...
/obj/machinery/power/muon_beam/emp_act(var/severity)//Emitters are hardened but still might have issues
	if((severity == 1) && prob(50))
		if(src.active)
			src.burst_shots /= 2
	return 1

/obj/machinery/power/muon_beam/process()
	if(stat & (BROKEN))
		return
...
/obj/machinery/power/muon_beam/attackby(obj/item/W, mob/user)
